    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="ai-generator-card">
            ...
            <div class="ai-input-group">
                <textarea id="aiPrompt" class="ai-textarea" placeholder="Например: Создай договор аренды квартиры на 11 месяцев между Иванов Иван Иванович и Петров Петр Петрович, сумма 50000 руб/мес..." rows="3"></textarea>
            ...
            <button type="button" id="generateBtn" class="ai-generate-btn"><i class="bi bi-magic"></i> <span>Сгенерировать с ИИ</span></button>
            <div id="aiStatus" class="ai-status hidden"><div class="ai-status-icon"><i class="bi bi-hourglass-split"></i></div><div class="ai-status-text">ИИ генерирует документ...</div></div>
            <div id="aiQuestions" class="ai-questions hidden">
                <div class="questions-title"><i class="bi bi-question-circle"></i> ИИ задаёт уточняющие вопросы:</div>
                <div id="questionsList"></div>
                <button type="button" id="submitAnswers" class="ai-submit-btn"><i class="bi bi-check-circle"></i> Отправить ответы</button>
            </div>
            <div id="aiResult" class="ai-result hidden">
                <div class="result-header"><i class="bi bi-check-circle-fill"></i> Документ успешно сгенерирован!</div>
                <div class="result-actions"><a id="downloadLink" href="#" class="download-btn" download><i class="bi bi-download"></i> Скачать документ</a></div>
            </div>
            ...
            <form action="{{ route('documents.store') }}" method="POST" enctype="multipart/form-data" id="documentForm">
                ...
                <div class="field-row">
                    <div class="field-group">
                        ...
                        <input type="text" name="number" id="field-number" class="input-field" value="{{ old('number', '№ ') }}" data-i18n-placeholder="numberPlaceholder" placeholder="№ 001" required>
                    </div>
                    <div class="field-group">
                        <label class="field-label"><span data-i18n="labelType">Тип документа</span> <span class="required">*</span></label>
                        <input type="text" name="type" id="field-type" class="input-field" data-i18n-placeholder="typePlaceholder" placeholder="Договор, Акт..." value="{{ old('type') }}" required>
                    </div>
                </div>
                ...
                <div class="field-row single">
                    <div class="field-group">
                        ...
                        <input type="text" name="title" id="field-title" class="input-field" data-i18n-placeholder="titlePlaceholder" placeholder="Название документа" value="{{ old('title') }}" required>
                    </div>
                </div>
                ...
                <div class="field-row single">
                    <div class="field-group">
                        ...
                        <textarea name="content" id="field-content" rows="3" class="input-field" data-i18n-placeholder="descriptionPlaceholder" placeholder="Краткое описание документа...">{{ old('content') }}</textarea>
                    </div>
                </div>

                <div class="field-row single">
                    <div class="field-group">
                        <label class="field-label"><span data-i18n="labelFile">Прикрепить файл</span> <span class="required">*</span></label>
                ...
                <div class="receiver-section">
                    ...
                    <div id="mode-by_region" class="receiver-block hidden">
                        ...
                        <a href="{{ route('documents.recipients.create', ['return_url' => route('documents.create')]) }}" class="byregion-add-btn">
                    ...
                    <div id="mode-by_department" class="receiver-block hidden">
                        ...
                        <a href="{{ route('documents.select-by-department', ['return_url' => route('documents.create')]) }}" class="byregion-add-btn dept-add-btn">
                    ...
                    {{-- ✅ БЛОК 6: ИЗ ДЕРЕВА КОМПАНИЙ (С КНОПКОЙ X) --}}
                    @php
                        $selCompanyId = session('selected_company_id');
                        $selCompany = $selCompanyId ? \App\Models\Company::find($selCompanyId) : null;
                        $selCompanyUsers = session('selected_company_users', []);
                        $companyUserCount = count($selCompanyUsers);
                    @endphp
                    <div id="mode-by_company" class="receiver-block hidden">
                        ...
                        <a href="{{ route('documents.select-by-company', ['return_url' => route('documents.create')]) }}" class="byregion-add-btn company-add-btn">
                            <i class="bi bi-folder-plus"></i>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const FORM_STORAGE_KEY = 'docsign_draft_create_form';
        ...
        const translations = {
            ru: {
                back: "Назад", errorTitle: "Ошибка при создании документа", pageTitle: "Новый документ",
                pageSubtitle: "Заполните информацию о документе", labelNumber: "Номер документа", labelType: "Тип документа",
                labelStatus: "Статус документа", labelDeadline: "Дедлайн", labelTitle: "Заголовок", labelDescription: "Описание",
                labelFile: "Прикрепить файл", labelReceiverMode: "Способ отправки", modeAllTeam: "Всей команде",
                ...
                searchCompanyLabel: "Поиск получателя из другой команды", submitButton: "Создать документ",
                filePlaceholder: "Выберите файл...", usersNotFound: "Пользователи не найдены", maxReceivers: "Максимум 5 получателей",
                ...
                fileRequiredError: "❌ Файл не выбран! Пожалуйста, прикрепите файл для отправки.",
            ...
            tj: {
                back: "Бозгашт", errorTitle: "Хато ҳангоми эҷоди ҳуҷҷат", pageTitle: "Ҳуҷҷати нав",
                pageSubtitle: "Маълумот оид ба ҳуҷҷатро пур кунед", labelNumber: "Рақами ҳуҷҷат", labelType: "Намуди ҳуҷҷат",
                labelStatus: "Ҳолати ҳуҷҷат", labelDeadline: "Мӯҳлат", labelTitle: "Сарлавҳа", labelDescription: "Тавсиф",
                labelFile: "Файл замима кардан", labelReceiverMode: "Усули фиристодан", modeAllTeam: "Ба ҳамаи даста",
                ...
                searchCompanyLabel: "Ҷустуҷӯи гиранда аз дигар даста", submitButton: "Эҷоди ҳуҷҷат",
                filePlaceholder: "Файлро интихоб кунед...", usersNotFound: "Корбарон ёфт нашуданд", maxReceivers: "Ҳадди аксар 5 гиранда",
                ...
                fileRequiredError: "❌ Файл интихоб нашудааст! Лутфан, барои фиристодан файлро замима кунед.",
            ...
            en: {
                back: "Back", errorTitle: "Error creating document", pageTitle: "New Document",
                pageSubtitle: "Fill in the document information", labelNumber: "Document Number", labelType: "Document Type",
                labelStatus: "Document Status", labelDeadline: "Deadline", labelTitle: "Title", labelDescription: "Description",
                labelFile: "Attach File", labelReceiverMode: "Sending Method", modeAllTeam: "All Team",
                ...
                searchCompanyLabel: "Search recipient from another team", submitButton: "Create Document",
                filePlaceholder: "Choose file...", usersNotFound: "Users not found", maxReceivers: "Maximum 5 recipients",
                ...
                fileRequiredError: "❌ File not selected! Please attach a file to submit.",
        ...
        // =====================================================================
        // ✅ ФУНКЦИЯ ОЧИСТКИ СЕССИИ КОМПАНИИ (ИСПОЛЬЗУЕТСЯ В 2 МЕСТАХ)
        // =====================================================================
        ...
        function saveFormToStorage() {
            ...
            const formData = {
                ...
                fileName: localStorage.getItem('docsign_last_file_name') || '',
                timestamp: Date.now()
            };
        ...
        function clearFormStorage() {
            ...
            localStorage.removeItem('docsign_last_file_name');
        }

        const teamUsers = @json($teamUsersArray ?? []);
        const otherUsers = @json($otherUsersArray ?? []);
        const CARRIED_IDS = @json(session('selected_recipients') ?? []);
            ...
            fileInput.addEventListener('change', function() {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        localStorage.setItem('docsign_last_file_name', data.file_name);
        ...
        // =====================================================================
        // ✅ ИСПРАВЛЕННАЯ ВАЛИДАЦИЯ: ПЕРЕД SUBMIT ОЧИЩАЕМ СЕССИЮ КОМПАНИИ
        // =====================================================================
        form.addEventListener('submit', async function(e) {
            ...
            if (!hasError && !hasFileInInput && !hasTempFile) {
            ...
            if (!hasError) {
                // ✅ ВАЖНО: Очищаем сессию компании ПЕРЕД отправкой формы
                // Это гарантирует что при следующем создании документа старые данные не останутся
                try {
                    await clearCompanySession();
                    console.log('✓ Сессия компании очищена перед отправкой документа');
                } catch (err) {
        ...
        // =====================================================================
        // ✅ ИСПРАВЛЕННАЯ ОЧИСТКА ВЫБОРА КОМПАНИИ (КНОПКА X)
        // =====================================================================
        ...
        if (generateBtn) {
            generateBtn.addEventListener('click', async function() {
                ...
                if (!prompt) { showError('Введите описание документа'); return; }
